...
const ComparisonTable: React.FC<ComparisonTableProps> = ({ items, ctaLabel, onCta, labels }) => {
  return (
    <section className="rounded-2xl bg-white dark:bg-[#111827] shadow-sm border border-[#E5E7EB] dark:border-white/10 overflow-hidden">
      <div className="px-6 py-4 border-b border-[#E5E7EB]">
        <h3 className="text-lg font-black text-[#111827] dark:text-white">{labels.title}</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-[#F9FAFB] dark:bg-[#0f172a] text-[#6B7280] text-xs">
            <tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b last:border-none">
                <td className="px-4 py-4 font-bold text-[#111827] dark:text-white">{item.rank}</td>
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="font-semibold text-[#111827] dark:text-white">{item.name}</div>
                  </div>
                </td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-gray-600 dark:text-gray-300">{item.priceRangeText || '—'}</td>
                <td className="px-4 py-4 text-gray-600 dark:text-gray-300">{item.bestFor}</td>
                <td className="px-4 py-4">
                  <button
